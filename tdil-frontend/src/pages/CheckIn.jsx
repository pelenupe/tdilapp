...
export default function CheckIn() {
  ...
  const getMyLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        // 1. Reverse geocode — use backend proxy to avoid exposing API key
        try {
          ...
          if (geocodeResponse.ok) {
            ...
            setLocation(geocodeData.address || `${latitude}, ${longitude}`);
          } else {
        ...
        // 2. Nearby places — independent fetch so geocode failure doesn't block this
        try {
          const token = localStorage.getItem('token');
          ...
          if (placesData.results && placesData.results.length > 0) {
            setNearbyPlaces(placesData.results.slice(0, 10));
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className={`overflow-auto pt-16 md:pt-0 transition-all duration-300 ${isCollapsed ? 'md:ml-16' : 'md:ml-64'}`}>
