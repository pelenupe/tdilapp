...
router.post('/', protect, async (req, res) => {
  try {
    const { name, description, price, points_price, image_url, category, stock } = req.body;
    ...
    const result = await query(
      `INSERT INTO merch (name, description, price, points_cost, image_url, category, stock_quantity, is_active, created_by, created_at) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, 1, $8, CURRENT_TIMESTAMP) RETURNING id`,
      [name, description || null, price || 0, points_price || null, image_url || null, category || 'apparel', stock || 10, req.user.id]
    );
...
router.put('/:id', protect, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, description, price, points_price, image_url, category, stock, is_active } = req.body;
    const userType = req.user.userType;
    ...
    await query(
      `UPDATE merch SET 
       ...
       WHERE id = $9`,
      [name, description, price, points_price, image_url, category, stock, is_active, id]
    );
