...
const generateToken = async (req, res) => {
  try {
    ...
    await query(
      'INSERT INTO invite_tokens (token, email, createdBy, expires_at, user_type, is_reusable) VALUES ($1, $2, $3, $4, $5, $6)',
...
const validateToken = async (req, res) => {
  try {
    ...
    // Check DB for token (works with both old and new token schemas)
    const result = await query(
      `SELECT * FROM invite_tokens 
       WHERE token = $1 
       AND (is_used = 0 OR is_used = FALSE OR is_active = 1)
    ...
    return res.json({
      valid: true,
      email: row.email,
      userType: row.userType || row.user_type || 'member',
      isReusable: row.is_reusable || row.is_active || false,
...
const getTokenInfo = async (token) => {
  try {
    // Query DB — supports both old schema (is_used/user_type) and new schema (is_active/userType)
    const result = await query(
      `SELECT * FROM invite_tokens 
       WHERE token = $1 
       AND (is_used = 0 OR is_used = FALSE OR is_active = 1)
...
const getAllTokens = async (req, res) => {
  try {
    ...
    const tokens = await query(`
      ...
      LEFT JOIN users creator ON it.createdBy = creator.id
