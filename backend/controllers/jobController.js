...
const postJob = async (req, res) => {
  try {
    const { title, description, location, company, salary, jobType, requirements, benefits } = req.body;
    ...
    const result = await query(
      `INSERT INTO jobs (title, description, location, company, salaryRange, jobType, requirements, benefits, postedBy, createdAt) 
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, CURRENT_TIMESTAMP) RETURNING id`,
      [title, description || null, location || null, company, salary || null, jobType || 'full-time', requirements || null, benefits || null, req.user.id]
    );

    const jobId = result[0]?.id;

    // Award points for posting a job
    try { await awardPoints(req.user.id, 'JOB_POST', `Posted job: ${title}`, { jobId }); } catch(e) {}
...
const getJob = async (req, res) => {
  try {
    ...
    res.json({
      ...
      requirements: job.requirements,
      benefits: job.benefits,
      contactEmail: job.postedByEmail,
      postedBy: job.postedBy,
      postedByName: job.postedByName,
      postedByCompany: job.postedByCompany,
      createdAt: job.createdAt,
...
const deleteJob = async (req, res) => {
  try {
    ...
    const job = await query('SELECT postedBy FROM jobs WHERE id = $1', [id]);
    
    if (job.length === 0) {
      return res.status(404).json({ message: 'Job not found' });
    }

    if (job[0].postedBy !== userId && userType !== 'admin' && userType !== 'founder') {
  ...
  try {
    ...
    const result = await query(
      `INSERT INTO job_applications (user_id, job_id, cover_letter, status, created_at) 
       VALUES ($1, $2, $3, 'pending', CURRENT_TIMESTAMP) RETURNING id`,
      [req.user.id, jobId, coverLetter || null]
    );

    try { await awardPoints(req.user.id, 'JOB_APPLICATION', `Applied to: ${job[0].title}`, { jobId }); } catch(e) {}
...
const getMyJobs = async (req, res) => {
  try {
    const jobs = await query(
      `SELECT * FROM jobs WHERE postedBy = $1 ORDER BY createdAt DESC`,
      [req.user.id]
    );
